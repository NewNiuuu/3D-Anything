"""
Qwen3.5-35B-A3B 多模态推理脚本
=====================================
用途：通过 vLLM OpenAI 兼容 API，批量对图片进行多模态推理。
      适用于 visual grounding CoT 数据生成等任务。

模型信息：Qwen3.5-35B-A3B 是多模态 MoE 模型（256 experts, 8 active），
         支持文本 + 图像 + 视频输入，性能优于 Qwen2-VL-72B-Instruct。

使用前提：
    先启动 vLLM 服务：bash start_qwen35.sh
...
用法：
    ...
    # 批量处理整个目录
    python scripts/infer_qwen35.py --image-dir data/DVG_sample --max-images 10

    # 结合 JSON 标注信息（将标注中的 question 作为提示词）
    python scripts/infer_qwen35.py --image-dir data/DVG_sample --use-annotation --max-images 5

    # 自定义输出路径
    python scripts/infer_qwen35.py --image-dir data/DVG_sample --output results/output.jsonl
"""
...
def infer_single_image(
    ...
    Qwen3.5-35B-A3B 内置视觉编码器，支持图像理解。
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Qwen3.5 推理脚本 — 支持单图/批量模式",
...
def main():
    ...
    try:
        ...
        print("  请确认已运行: bash start_qwen35.sh")
        sys.exit(1)
